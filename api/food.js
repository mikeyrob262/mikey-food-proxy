...
export default {
  ...
  // WHICH BASIS A ROW'S NUMBERS ARE ON, and what to call it. Split out and named so the decision is
  // testable on its own and so the mapper below has exactly one place to ask.
  //
  // Returns { factor, label, per100 }. factor is 1 whenever the serving cannot be honestly
  // converted, so an unconvertible row keeps its per-100 g values rather than being scaled by a
  // guess — and its label says per-100 g so the caller is never told a serving it is not getting.
  const FDC_MASS_UNITS = {g:1, grm:1, gm:1, gram:1, grams:1, ml:1, mlt:1, milliliter:1, millilitre:1, milliliters:1}
  function fdcServing(p) {
    const sz = parseFloat(p && p.servingSize)
    const unit = String((p && p.servingSizeUnit) || '').trim().toLowerCase()
    if (sz > 0 && FDC_MASS_UNITS[unit]) {
      const isVol = unit.charAt(0) === 'm'
      return { factor: sz / 100, label: Math.round(sz) + (isVol ? 'ml' : 'g'), per100: false }
    }
    // A serving in ounces, pieces or cups: there is no factor from per-100 g without knowing the
    // density or the piece mass, and inventing one is exactly what this fix exists to stop. Say the
    // basis, and carry the label serving as context rather than as the thing being reported.
    if (sz > 0) return { factor: 1, label: '100g (label serving ' + Math.round(sz) + ' ' + unit + ')', per100: true }
    return { factor: 1, label: '100g', per100: true }
  }
  ...
  try {
    ...
    // exactly how two definitions of one number start. fdcServing (top of file) decides the basis.
    //
    // FDC VALUES ARE PER 100 g AND WE WERE LABELLING THEM AS A SERVING.
    //
    // foodNutrients[].value is per 100 g for every FDC dataType. servingSize/servingSizeUnit
    // describe the LABEL serving and were read only to build the display string, so a row arrived
    // with per-100 g macros under a "32g" heading. Logging one Quest protein-chip bag recorded
    // 375 cal / 65.6 g protein against a real 140 / 19.
    //
    // AND THE ERROR FLIPS WITH SERVING SIZE, which is why it was reported as "understating": an
    // OWYN shake is a ~354 g serving, so the same unscaled per-100 g figure UNDER-reports it by
    // about the same factor a 32 g bag is OVER-reported by. It was never systematically understating
    // — it was unscaled, and the direction just followed whether the serving was above or below 100 g.
    //
    // Only grams and millilitres are scaled: per-100 g is directly comparable to those and to
    // nothing else. A serving measured in ounces, pieces or cups has no honest factor here, so those
    // rows keep per-100 g values and SAY so rather than borrowing the label's serving text.
    //
    // Rows with no servingSize at all (Foundation, SR Legacy, Survey/FNDDS) are genuinely per-100 g.
    // They were being labelled "1 serving", which is the same misstatement in a quieter voice — and
    // it is the majority of the generic-ingredient results, not an edge case.
    const mapFdc = (list) => (list||[]).map(p => {
        const _sv = fdcServing(p)
        ...
        ;(p.foodNutrients||[]).forEach(n => {
          ...
          // COLLECTED RAW, ROUNDED ONCE AT THE END. Rounding here and then scaling would round
          // twice — a 0.05 g rounding on a per-100 g protein becomes 0.18 g after a 3.5x serving
          // factor, which is the kind of drift that makes a logged day quietly disagree with itself.
          if(id==1008||id=='208') cal=v
          else if(id==2048||id=='957') calAtwaterSpecific=v
          else if(id==2047||id=='956') calAtwaterGeneral=v
          else if(id==1062||id=='268') kJ=v
          else if(id==1003||id=='203') pro=v
          else if(id==1005||id=='205') carb=v
          else if(id==1004||id=='204') fat=v
          else if(id==1079||id=='291') fiber=v
          else if(id==2000||id=='269') sugar=v
          else if(id==1093||id=='307') sodium=v
          else if(id==1258||id=='606') satFat=v
        })
        ...
        if(!cal) cal = calAtwaterSpecific || calAtwaterGeneral || (kJ ? kJ/4.184 : 0)
        if(!cal && p.dataType==='Branded') return null
        // Scale the per-100 g figures onto the serving this row is going to be LABELLED with, then
        // round. _sv.factor is 1 whenever the serving cannot be honestly converted, so an
        // unconvertible row is unchanged in value and simply labelled per-100 g.
        const _f = _sv.factor
        const _r1 = (x) => Math.round((x||0)*_f*10)/10
        cal=Math.round(cal*_f); pro=_r1(pro); carb=_r1(carb); fat=_r1(fat)
        fiber=_r1(fiber); sugar=_r1(sugar); satFat=_r1(satFat); sodium=Math.round((sodium||0)*_f)
