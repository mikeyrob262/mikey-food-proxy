...
export default {
  ...
  try {
    // 1. Search local curated database first (instant, no rate limits)
    const localResults = searchLocalFoods(q);

    // 2. If we have enough good local matches, return immediately
    if (localResults.length >= 5) {
      return new Response(JSON.stringify({foods: localResults}), {headers})
    }

    // 3. Otherwise supplement with USDA API
