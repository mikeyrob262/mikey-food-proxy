  ...
  try {
    ...
    if (offRes.status === 'fulfilled' && offRes.value.ok) {
      ...
      const offFoods = (offData.products||[]).map(p => {
        ...
        const pro = Math.round((nu['proteins_serving'] || nu['proteins_100g'] || 0)*10)/10
        const carb = Math.round((nu['carbohydrates_serving'] || nu['carbohydrates_100g'] || 0)*10)/10
        const fat = Math.round((nu['fat_serving'] || nu['fat_100g'] || 0)*10)/10
        if(!cal) return null
        return {
